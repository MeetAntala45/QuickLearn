...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { name: 'Students Enrolled', icon: assets.person_tick_icon, path: '/educator/student-enrolled' }
  ];

  return isEducator && (
    <div className='md:w-64 w-16 border-r min-h-screen text-base border-gray-500 py-2 flex flex-col bg-gradient-to-b from-gray-100/70'>
      {menuItems.map((item) => (
        <NavLink
          to={item.path}
          key={item.name}
        ...
        >
          <img src={item.icon} alt="" className='h-6 w-' />
